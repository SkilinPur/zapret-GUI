...
class HelpTab(QWidget):
    ...
    def _build_ui(self):
        ...
        ql.addWidget(_step(
            1,
            "Стратегии и ядро уже в комплекте — ничего скачивать не нужно. "
            "Проверить/обновить версии программы, ядра nfqws и стратегий можно "
        ...
        tl.addWidget(_bullet(
            "<b>Конфигурация</b> — стратегия (с описанием под выбором), "
            "интерфейс, бэкенд фаервола, GameFilterTCP/UDP. Сохраняется в conf.env.",
        ))
        tl.addWidget(_bullet(
            "<b>Обновление</b> — версии и обновление программы, ядра nfqws и "
            "стратегий Flowseal, история изменений.",
        ))
        tl.addWidget(_bullet(
        ...
        tl2.addWidget(_bullet(
            "Убедитесь, что nfqws скачан (вкладка «Обновление» → «Ядро nfqws»).",
        ))
        tl2.addWidget(_bullet(
